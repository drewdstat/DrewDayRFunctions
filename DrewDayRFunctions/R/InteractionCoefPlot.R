...
#' @param robust This is an option to use HC0 robust sandwich errors for the CIs
...
#' @param xlablog10 An option to change the x-axis labels to be of the form 
#' "10^x". This is useful if your predictor of interest (or also the \code{ixterm}
...
InteractionCoefPlot <- function(model, data, pred, ixterm, multiplier = 1, 
  ...
  predcoefdat<-data.frame(Outcome=outname,Predictor=predname,Interaction=ixname,
                          IxLevel=seq(min(data[,ixterm],na.rm=T),
