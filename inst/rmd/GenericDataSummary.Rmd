...
summ2 <- data.frame(Variable = contvars, Mean = colMeans(as.data.frame(Data[, contvars])), SD = sapply(as.data.frame(Data[, contvars]), sd))
datatable(summ2, caption = "Continuous Covariate Summary Statistics", 
          rownames = F, options = list(pageLength = 10, rowsGroup = list(0), 
                                  initComplete  =  htmlwidgets::JS(
          "function(settings,  json) {", 
          "$(this.api().table().header()).css({'font-size': '12px'});", "}")))%>%
  formatStyle(columns = colnames(summ2), fontSize = '12px')
kable(summ)
kable(summ2)
```
