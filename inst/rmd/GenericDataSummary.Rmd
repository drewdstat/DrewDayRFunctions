...
```{r contcorr_pre, results='asis'}
if(length(contvars) > 30){
  nocontcorrtag <- TRUE
  cat("There are too many continuous variables to plot correlation coefficients.")
} else nocontcorrtag <- FALSE
```
```{r contcorr}
if(!nocontcorrtag) mypearson(Data = Data[, contvars])
```

### Missingness

```{r contmiss}
misspercdt(Data[, contvars])
```

## Categorical Summary Statistics {.tabset}
...
```{r catplot}
catplot <- catpercviz(Data[, catvars])
